...
{
    /* 
     * LinqSudokuSolver; edited.
     * Search for "EDIT:" to see what's edited.
     */
    ...
    //EDIT: Class renamed; most static members are made non-static and public. 
    class Norvig : SearchAlgorithm<string>
    {
        //EDIT: added fields for analysis:
        Stopwatch st;
        int assigns;
        ...
        // EDIT: The pre-defined fields below are now defined in the constructor.

        public string rows;
        public string cols;
        public string digits;
        public string[] squares;
        public Dictionary<string, IEnumerable<string>> peers;
        public Dictionary<string, IGrouping<string, string[]>> units;
        
        // EDIT: Fields added:
        public int N;
        public int sqrN;
                
        // EDIT: Renamed constructor.
        public Norvig(short N)
        {
            // EDIT: inserted block of code:   
            this.N = N;
            sqrN = N*N;

            string vals = "";
            for (short i = 1; i <= sqrN; i++) 
                vals += (char)i;
            rows = vals;
            cols = vals;
            digits = vals;

            string[] boxedRows = new string[N];
            for(int i = 0; i< N; i++) 
                boxedRows[i] = rows.Substring(i*N,N);
            string[] boxedCols = boxedRows;

            squares = cross(rows,cols);
            //END EDIT: inserted block of code.
            ...
            var unitlist = ((from c in cols select cross(rows, c.ToString()))
                      ...
                      // EDIT: .Concat(from rs in (new[] { "ABC", "DEF", "GHI" }) from cs in (new[] { "123", "456", "789" }) select cross(rs, cs)));
                               .Concat(from rs in boxedRows from cs in boxedCols select cross(rs, cs)));
            /*
        ...
        */
        ...
        public Dictionary<string, string> parse_grid2(string grid)
        {
            //EDIT: grid2 was never used!: var grid2 = from c in grid where "0.-123456789".Contains(c) select c;!
            var values = squares.ToDictionary(s => s, s => digits); //To start, every square can be any digit

            string[] nums = grid.Split(' ');

            foreach (var sd in zip(squares, nums))
            {
                var s = sd[0];
                //EDIT: translate value into char;
                short dNum;
                if (!short.TryParse(sd[1], out dNum))
                    dNum = 0;
                var d = "" + (char)dNum;
        ...
        // EDIT: Made to a normal method instead of an extention:
        ...
        // EDIT: Printing method changed. Return type from values to void.
        public void print_board(Dictionary<string, string> values)
        {
            if (values == null) return;

            // EDIT: Translate:
            List<string> keys = new List<string>(values.Keys);
            foreach (string square in keys)
                values[square] = ToNumbers(values[square]);

            var width = 1 + (from s in squares select values[s].Length).Max();
            // EDIT: N instead of 3:
            var line = "\n" + String.Join("+", Enumerable.Repeat(new String('-', width * N), N).ToArray());

            foreach (var r in rows)
            {
                Console.WriteLine(String.Join("",
                    (from c in cols
                     //EDIT: Center called in a different way, and different bar-/line check functions:
                     select Center(values["" + r + c], width) + ((short)c % N == 0 ? "|" : "")).ToArray())
                        + ((short)r % N == 0 ? line : ""));
            }

            Console.WriteLine();
        }
        // EDIT: Translating method added:
        private string ToNumbers(string p)
        {
            string nums = "";
            foreach (char c in p)
                nums += ((short)c) + " ";
...
3...8.......7....51..............36...2..4....7...........6.13..452...........8..".Split('\n');
    ...
    public class NorvigSudokuString : SearchProblem<string>
    {
        ...
        public int Heuristic(string s)
        {
            throw new NotImplementedException();
        }

        public bool IsGoal(string solution)
        {
            throw new NotImplementedException();
        }
    }
